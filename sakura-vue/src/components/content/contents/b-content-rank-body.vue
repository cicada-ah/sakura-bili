<template>
	<div class="b-body">
		<div class="r-list-body">
			<div class="r-list-wrapper" ref="listWrapper">
				<ul class="rlist" v-if="rank.hot">
					<li 
					:class="{first: index === 0}" 
					v-for="(item, index) in rank.hot.list"
					:key="index"
					>
						<i class="number" :class="{n: index<3}">
							{{index + 1}}
						</i>
						<div class="preview">
							<a href="/" :title="item.title" target="_blank">
								<img 
								v-lazy="item.pic" style="opacity: 1;">
								</a>
						</div>
						<a class="rl-info" href="/" :title="item.title + item.pts" target="_blank">	
				...
				<ul class="rlist" v-if="rank.hot_original">
					<li 
					:class="{first: index === 0}" 
					v-for="(item, index) in rank.hot_original.list"
					:key="index"
					>
						<i class="number" :class="{n: index<3}">
							{{index + 1}}
		...
		<div class="more-link">
			<a href="/ranking#!/all/2/2/2/" target="_blank">查看更多</a>
		</div>		
	</div>
</template>

<script>
import { contentrankApi } from 'src/bajax/ajax' 
export default {
	data() {
		return {
			threeDayData: {},
	...
	watch: {
		isOrigin(val, oldVal) {
			val ? this.$refs.listWrapper.style.marginLeft = '-100%':
	...
	methods: {
		getRankData() {
			...
			if (this.isWeek && JSON.stringify(this.weekData) != "{}") {
				...
				return
			} else if (!this.isWeek && JSON.stringify(this.threeDayData) != "{}") {
